...
void FFTWThread::run() {
    while (!player->error_code) {
        ...
        for (int32_t i = std::max(0, player->rgb_parameters.red_peak - player->rgb_parameters.width);
             i < std::min(player->msg.sample_size, player->rgb_parameters.red_peak + player->rgb_parameters.width);
             ++i) {
